...
use crate::bridge::{Bridge, BridgeAction, BridgeError, BridgeEvent, BridgeStatus};
use crate::ids::{DeviceId, HostId, InterfaceId};
use crate::input_profile::{InputIdentity, InputProfileId, InputProfileRegistry};
#[cfg(feature = "dual-s3-wired")]
use crate::interchip::{
    ActivateProfile, MirrorControlRequest, MirrorControlResponse, ProfileResult,
    ProfileResultStatus, RawEndpointReport,
};
use crate::management::{
    ManagementDestination, ManagementDiagnostics, ManagementHistoryEvent, ManagementRequest,
};
#[cfg(feature = "dual-s3-wired")]
...
#[cfg(feature = "dual-s3-wired")]
pub use command::DeviceTaskCommand;
pub use command::{
    BleCommandLane, BleTaskCommand, CommandClass, ManagementTaskResponse, RuntimeCommand,
    RuntimeEffect, StatusTaskCommand, StorageTaskCommand, UsbHostTaskCommand,
};
use diagnostics::saturating_depth;
pub use diagnostics::{RuntimeCounters, RuntimeDiagnosticsEvent, RuntimeTransportMetrics};
pub use queues::{RuntimeCommandQueues, RuntimeDispatchError};
...
pub struct BridgeRuntime<const HOSTS: usize, const USB_INTERFACES: usize> {
    ...
    #[cfg(feature = "dual-s3-wired")]
    pending_mirror_candidate: Option<(
        MirrorCandidateId,
        MirrorStableId,
        u32,
        bool,
...
impl<const HOSTS: usize, const USB_INTERFACES: usize> BridgeRuntime<HOSTS, USB_INTERFACES> {
    ...
    #[cfg(feature = "dual-s3-wired")]
    fn physical_usb_device_count(&self) -> usize {
        self.usb_interfaces
            .iter()
            .enumerate()
            .filter_map(|(index, interface)| {
                let device_id = interface.as_ref()?.device_id;
                (!self.usb_interfaces[..index]
                    .iter()
                    .flatten()
                    .any(|previous| previous.device_id == device_id))
                .then_some(())
            })
            .count()
    }
    ...
    #[cfg(feature = "dual-s3-wired")]
    fn effective_mirror_candidate(&self) -> Option<MirrorCandidateId> {
        let candidate = self.selected_mirror_candidate()?;
        let physical_source = self
            .mirror_candidates
            .get(candidate)
            .is_some_and(|metadata| metadata.source_device.is_some());
        (!physical_source || self.physical_usb_device_count() <= 1).then_some(candidate)
    }

    #[cfg(feature = "dual-s3-wired")]
    fn reconcile_wired_presentation<const COMMANDS: usize>(
        &mut self,
        ...
        let desired = self.effective_mirror_candidate();
        let current = if self.presentation_transition_pending {
            self.transition_mirror_target
        } else {
            self.active_mirror_target
        };
        if current != desired {
            self.begin_presentation_transition(desired, commands)?;
        }
        Ok(())
    }
    ...
    fn activate_selected_mirror_candidate<const COMMANDS: usize>(
        ...
        if self.effective_mirror_candidate() == Some(candidate)
        ...
        match input {
            ...
            RuntimeInput::DeviceProfileResult(result) => {
                ...
                if self
                    ...
                    .is_some_and(|(_, _, profile_hash, _, _)| profile_hash == result.profile_hash)
                {
                    if matches!(
                        result.status,
                        ProfileResultStatus::Accepted | ProfileResultStatus::AlreadyStored
                    ) && let Some((candidate, stable_id, profile_hash, synthetic, source_device)) =
            ...
            #[cfg(feature = "dual-s3-wired")]
            RuntimeInput::DeviceUsbState(state) => {
                commands.clear();
            ...
            RuntimeInput::MirrorCandidateRegistered {
                ...
                if let Some(profile_hash) = profile_hash {
                    if self.last_profile_result.is_some_and(|result| {
                        ...
                        self.pending_mirror_candidate =
                            Some((candidate, stable_id, profile_hash, synthetic, source_device));
                    }
                } else {
            ...
            RuntimeInput::MirrorSourceDisconnected { device_id } => {
                commands.clear();
                self.mirror_candidates.clear_source(device_id);
                if self
                    .pending_mirror_candidate
                    .is_some_and(|(_, _, _, _, source)| source == Some(device_id))
                {
                    self.pending_mirror_candidate = None;
                }
                self.reconcile_wired_presentation(commands)
            }
    ...
    fn dispatch_bridge_action<const COMMANDS: usize>(
        ...
        match action {
            ...
            BridgeAction::ActivateWired { operation_id } => {
                let target = self.effective_mirror_candidate();
...
mod tests {
    ...
    use crate::management::{
        ManagementCommand, ManagementHostInfo, ManagementHostName, ManagementHostStatus,
        ManagementResponse, ManagementResponsePayload, ManagementResult, ManagementSetting,
        ManagementStatus, ManagementUsbStatus,
    };
    #[cfg(feature = "dual-s3-wired")]
    use crate::management::{ManagementOutputTarget, ManagementUsbPresentationKind};
    #[cfg(feature = "dual-s3-wired")]
    use crate::output_target::StoredMirrorTarget;
    ...
    #[cfg(feature = "dual-s3-wired")]
    #[test]
    fn restored_missing_mirror_target_accepts_ready_fallback_device() {
        let mut runtime = BridgeRuntime::<4, 1>::new(0);
        let mut commands = heapless::Vec::<RuntimeCommand, 16>::new();
        ...
        runtime
            .restore_storage_state(&storage, &mut commands)
            .unwrap();
        ...
        let status = runtime.management_output_target_status();
        assert_eq!(status.active, Some(ManagementOutputTarget::Wired));
        assert!(status.wired_ready);
        assert_eq!(
            status.effective_presentation,
    ...
    #[cfg(feature = "dual-s3-wired")]
    #[test]
    fn disconnected_mirror_source_transitions_to_ready_fallback() {
        let mut runtime = BridgeRuntime::<4, 1>::new(0);
        let mut commands = heapless::Vec::<RuntimeCommand, 16>::new();
        let candidate = MirrorCandidateId(0);
        let source_device = DeviceId(7);
        let profile_hash = 0x4455_6677;
        runtime
            .handle_input::<16, 16, 2>(
                RuntimeInput::MirrorCandidateRegistered {
                    candidate,
                    stable_id: MirrorStableId::new(0x046d, 0xc547, None, profile_hash, &[1])
                        .unwrap(),
        ...
        runtime
            .handle_input::<16, 16, 2>(
                management_request(ManagementCommand::SetMirrorTarget(candidate), 1),
                &mut commands,
            )
            .unwrap();
        runtime
        ...
        assert_eq!(runtime.transition_mirror_target, None);
        ...
        runtime
            ...
            .unwrap();
        assert!(runtime.management_output_target_status().wired_ready);
        assert!(!runtime.presentation_transition_pending);
    }

    #[cfg(feature = "dual-s3-wired")]
    #[test]
    fn mirror_selection_requires_registered_candidate_and_activates_asynchronously() {
        let mut runtime = BridgeRuntime::<4, 1>::new(0);
        let mut commands = heapless::Vec::<RuntimeCommand, 16>::new();
        let candidate = MirrorCandidateId(0);
        ...
        runtime
            .handle_input::<16, 16, 2>(
                management_request(ManagementCommand::SetMirrorTarget(candidate), 30),
                &mut commands,
            )
            .unwrap();
        assert_eq!(
        ...
        runtime
            .handle_input::<16, 16, 2>(
                management_request(ManagementCommand::SetMirrorTarget(candidate), 31),
                &mut commands,
            )
            .unwrap();
        ...
        runtime
            .handle_input::<16, 16, 2>(
                management_request(ManagementCommand::ClearMirrorTarget, 32),
                &mut commands,
            )
            .unwrap();
        assert!(commands.iter().any(|command| matches!(
        ...
        assert_eq!(runtime.bridge.state().mirror_target, None);
    }

    #[cfg(feature = "dual-s3-wired")]
    #[test]
    fn rejected_profile_does_not_replace_registered_mirror_candidate() {
        let mut runtime = BridgeRuntime::<4, 1>::new(0);
        let mut commands = heapless::Vec::<RuntimeCommand, 16>::new();
        let candidate = MirrorCandidateId(0);

        for (transfer_id, profile_hash, status) in [
            (1, 0x1111_1111, ProfileResultStatus::Accepted),
            (2, 0x2222_2222, ProfileResultStatus::InvalidImage),
        ] {
            runtime
                .handle_input::<16, 16, 2>(
                    RuntimeInput::MirrorCandidateRegistered {
        ...
        runtime
            .handle_input::<16, 16, 2>(
                management_request(ManagementCommand::SetMirrorTarget(candidate), 33),
                &mut commands,
            )
            .unwrap();
        assert!(commands.iter().any(|command| matches!(
    ...
    #[cfg(feature = "dual-s3-wired")]
    #[test]
    fn candidate_registration_accepts_profile_result_that_arrived_first() {
        let mut runtime = BridgeRuntime::<4, 1>::new(0);
        let mut commands = heapless::Vec::<RuntimeCommand, 16>::new();
        let candidate = MirrorCandidateId(0);
        runtime
        ...
        runtime
            .handle_input::<16, 16, 2>(
                management_request(ManagementCommand::SetMirrorTarget(candidate), 34),
                &mut commands,
            )
            .unwrap();
    ...
    fn changed_candidate_fingerprint_requires_explicit_reselection() {
        ...
        runtime
            .handle_input::<16, 16, 2>(
                management_request(ManagementCommand::SetMirrorTarget(candidate), 35),
                &mut commands,
            )
            .unwrap();
    ...
    #[cfg(feature = "dual-s3-wired")]
    #[test]
    fn removing_physical_mirror_source_keeps_selection_and_activates_fallback() {
        let mut runtime = BridgeRuntime::<4, 2>::new(0);
        let mut commands = heapless::Vec::<RuntimeCommand, 16>::new();
        let candidate = MirrorCandidateId(1);
        let stable_id = MirrorStableId::new(0x046d, 0xc547, None, 0x1234, &[2]).unwrap();
        ...
        assert_eq!(
            runtime.bridge.state().mirror_target,
            Some(StoredMirrorTarget(stable_id))
        );
        assert!(commands.contains(&RuntimeCommand::DeviceCommand(
            DeviceTaskCommand::ReleaseAll
        )));
    ...
    #[cfg(feature = "dual-s3-wired")]
    #[test]
    fn second_physical_input_switches_mirror_to_composite_fallback() {
        let mut runtime = BridgeRuntime::<4, 3>::new(0);
        let mut commands = heapless::Vec::<RuntimeCommand, 16>::new();
        let candidate = MirrorCandidateId(0);
        let profile_hash = 0x5566_7788;
        ...
        runtime
            .handle_input::<16, 16, 2>(
                RuntimeInput::UsbEndpointIn {
                    device_id: DeviceId(1),
                    report: RawEndpointReport::new(0x81, 1, &[4]).unwrap(),
                    standard: Some(crate::input::InputFrame::Standard(keyboard_input(
                        KeyUsage(4),
                    ))),
                },
                &mut commands,
    ...
    #[cfg(feature = "dual-s3-wired")]
    #[test]
    fn device_reboot_keeps_wired_inactive_until_saved_mirror_is_restored() {
        let mut runtime = BridgeRuntime::<4, 1>::new(0);
        let mut commands = heapless::Vec::<RuntimeCommand, 16>::new();
        let candidate = MirrorCandidateId(0);
        let profile_hash = 0x4455_6677;
    ...
    fn default_runtime_capacities_cover_worst_case_target_switch_commands() {
        ...
        assert!(matches!(commands[14], RuntimeCommand::StatusChanged(_)));
