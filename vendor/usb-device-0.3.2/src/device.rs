...
impl<B: UsbBus> UsbDevice<'_, B> {
    ...
    /// Returns whether a class has deferred the current EP0 request.
    pub fn control_deferred(&self) -> bool {
        self.control.deferred()
    }

    /// Completes a deferred device-to-host EP0 request.
    pub fn complete_control_in(&mut self, data: &[u8]) -> Result<()> {
        if !self.control.deferred() {
            return Err(UsbError::InvalidState);
        }
        self.control.accept_in(|buffer| {
    ...
    /// Completes a deferred host-to-device EP0 request.
    pub fn complete_control_out(&mut self) -> Result<()> {
        if !self.control.deferred() {
            return Err(UsbError::InvalidState);
        }
        self.control.accept_out()
    }

    /// Stalls a deferred EP0 request.
    pub fn reject_deferred_control(&mut self) -> Result<()> {
        if !self.control.deferred() {
            return Err(UsbError::InvalidState);
        }
        self.control.reject()
    ...
    fn control_in(&mut self, classes: &mut ClassList<'_, B>, req: control::Request) -> Result<()> {
        ...
        for cls in classes.iter_mut() {
            ...
            if !self.control.waiting_for_response() || self.control.deferred() {
                return Ok(());
            }
        }
