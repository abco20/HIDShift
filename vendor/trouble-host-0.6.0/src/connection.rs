...
impl<'stack, P: PacketPool> Connection<'stack, P> {
        ...
        T: ControllerCmdAsync<LeConnUpdate> + ControllerCmdSync<LeReadLocalSupportedFeatures>,
    {
        ...
        if self.role() == LeConnRole::Peripheral || cfg!(feature = "connection-params-update") {
            return self.update_connection_params_l2cap(stack, params).await;
        }
        Ok(())
    }

    /// Request connection parameters with the LE L2CAP signaling procedure.
    ///
    /// This explicit path is useful for peripheral controllers whose optional
    /// Link Layer procedure is rejected asynchronously by the central.
    pub async fn update_connection_params_l2cap<T>(
        &self,
        stack: &Stack<'_, T, P>,
        params: &RequestedConnParams,
    ) -> Result<(), BleHostError<T::Error>>
    ...
    {
        if self.role() != LeConnRole::Peripheral || !params.is_valid() {
