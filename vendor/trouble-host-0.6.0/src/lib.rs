...
pub mod prelude {
    ...
    #[cfg(feature = "gatt")]
    pub use crate::gatt::*;
    pub use crate::host::{ControlRunner, EventHandler, HostMetrics, Runner, RxRunner, TxObserverStage, TxRunner};
    pub use crate::l2cap::*;
    #[cfg(feature = "default-packet-pool")]
...
impl<'stack, C: Controller, P: PacketPool> Stack<'stack, C, P> {
    ...
    /// Immediately submit every PDU currently waiting in the outbound queue.
    pub async fn flush_outbound_with_tx_observer<F>(
        &self,
        mut observer: F,
    ) -> Result<usize, BleHostError<C::Error>>
    where
        F: FnMut(host::TxObserverStage, &[u8]),
    {
        let mut flushed = 0;
        while let Some((conn, pdu)) = self.host.connections.try_take_outbound() {
            self.host.submit_outbound_pdu(conn, pdu, &mut observer).await?;
            flushed += 1;
        }
        Ok(flushed)
    }

    pub(crate) async fn submit_immediate_pdu_with_tx_observer<F>(
        &self,
    ...
    where
        F: FnMut(host::TxObserverStage, &[u8]),
    {
        while let Some((conn, queued)) = self.host.connections.try_take_outbound() {
            self.host.submit_outbound_pdu(conn, queued, observer).await?;
        }
        observer(host::TxObserverStage::Prepared, pdu.as_ref());
        self.host
            .submit_outbound_pdu(connection.handle(), pdu, observer)
            .await
    }
