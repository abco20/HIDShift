    ...
    P: PacketPool,
{
    ...
    pub(crate) async fn submit_outbound_pdu<F>(
        &self,
        conn: ConnHandle,
        pdu: Pdu<P::Packet>,
        observer: &mut F,
    ...
    where
        F: FnMut(TxObserverStage, &[u8]),
    {
        observer(TxObserverStage::Dequeued, pdu.as_ref());
        match self.l2cap(conn, pdu.len() as u16, 1).await {
            Ok(mut sender) => {
                observer(TxObserverStage::CreditGranted, pdu.as_ref());
                if let Err(error) = sender.send(pdu.as_ref()).await {
                    warn!("[host] error sending outbound pdu");
                    return Err(error);
                }
                observer(TxObserverStage::Submitted, pdu.as_ref());
                Ok(())
            }
            Err(BleHostError::BleHost(Error::NotFound | Error::Disconnected)) => {
...
impl<'d, C: Controller, P: PacketPool> Runner<'d, C, P> {
    ...
    /// Run the host and observe outbound L2CAP PDU progress.
    pub async fn run_with_tx_observer<F>(&mut self, mut observer: F) -> Result<(), BleHostError<C::Error>>
    where
        F: FnMut(TxObserverStage, &[u8]),
        C: ControllerCmdSync<Disconnect>
            + ControllerCmdSync<SetEventMask>
            ...
            + ControllerCmdSync<ReadBdAddr>,
    {
        ...
        let tx_fut = self.tx.run_with_observer(&mut observer);
        pin_mut!(control_fut, rx_fut, tx_fut);
        match select3(&mut tx_fut, &mut rx_fut, &mut control_fut).await {
...
impl<'d, C: Controller, P: PacketPool> TxRunner<'d, C, P> {
    ...
    pub async fn run(&mut self) -> Result<(), BleHostError<C::Error>> {
        self.run_with_observer(&mut |_, _| {}).await
    }

    /// Run the transmit loop and observe outbound L2CAP PDU progress.
    pub async fn run_with_observer<F: FnMut(TxObserverStage, &[u8])>(
        &mut self,
        observer: &mut F,
        ...
        loop {
            ...
            host.submit_outbound_pdu(conn, pdu, observer).await?;
        }
    }
}

/// Progress of an outbound L2CAP payload through the host TX runner.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum TxObserverStage {
    /// A latency-sensitive PDU was assembled and is ready for direct submit.
    Prepared,
    /// The PDU was removed from the host outbound queue.
    Dequeued,
    /// The controller granted enough ACL packet credits for the PDU.
    CreditGranted,
    /// The complete PDU was written to the controller transport.
    Submitted,
}
