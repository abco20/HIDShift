...
use crate::{gatt, BleHostError, Controller, Error, PacketPool, Stack, MAX_INVALID_DATA_LEN};
...
impl<T: AsGatt + ?Sized> Characteristic<T> {
    ...
    /// Notify and submit the assembled PDU directly to the controller.
    ///
    /// Older queued PDUs are drained first to preserve ATT ordering.
    pub async fn notify_immediate<C, P, F>(
        &self,
        stack: &Stack<'_, C, P>,
    ...
    where
        C: Controller,
        P: PacketPool,
        F: FnMut(crate::host::TxObserverStage, &[u8]),
    {
        let value = value.as_gatt();
        let server = connection.server;
        server.set(self.handle, value)?;
        ...
        if !server.should_notify(raw, cccd_handle) {
            return Ok(());
        }
        ...
        let pdu = gatt::assemble(raw, crate::att::AttServer::Unsolicited(uns))?;
        stack.submit_immediate_pdu_with_tx_observer(raw, pdu, &mut observer).await
    }
