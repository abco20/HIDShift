    ...
    if gintsts.hcint() {
        ...
        for ch in 0..ch_count {
            if haint & (1 << ch) != 0 {
                ...
                trace!("otg-host: hcint ch={} raw={:#010x} -> events={:#06x}", ch, hcint.0, events,);
...
struct TransferCancellation<'a, 'd, T: pipe::Type, D: pipe::Direction> {
    channel: &'a Channel<'d, T, D>,
    armed: bool,
}

impl<'a, 'd, T: pipe::Type, D: pipe::Direction> TransferCancellation<'a, 'd, T, D> {
    fn new(channel: &'a Channel<'d, T, D>) -> Self {
...
impl<T: pipe::Type, D: pipe::Direction> Drop for TransferCancellation<'_, '_, T, D> {
    fn drop(&mut self) {
        if self.armed {
            self.channel.cancel_transfer();
        }
    }
}
...
impl<T: pipe::Type, D: pipe::Direction> Channel<'_, T, D> {
    fn configure_channel(&self, dir_in: bool, ep_type: EndpointType, pktcnt: u16, xfrsiz: u32, dpid: u8) {
        ...
        let ch = self.index;
        trace!(
            "otg-host: configure ch={} dir_in={} ep_type={} dad={} ep={} mps={} pktcnt={} xfrsiz={} dpid={} lsdev={} split={:?}",
            ch,
            dir_in,
            ep_type as u8,
            self.device_address,
        ...
        // Configure split routing before enabling the host channel. The ESP32-S3
        // FS controller reaches low-speed devices behind a full-speed hub with
        // HCCHAR.LSDEV/PRE, not a high-speed TT split transaction, so keep
        // HCSPLT clear in that topology.
        if let Some(split) = self.split {
            if self.state.fields.port_speed.load(Ordering::Acquire) == 2 {
                let hcsplt = (1u32 << 31)
                    | (((split.hub_addr() as u32) & 0x7f) << 7)
                    | ((split.port() as u32) & 0x7f);
                r.hcsplt(ch).write_value(hcsplt);
            } else {
                r.hcsplt(ch).write_value(0);
            }
        } else {
            r.hcsplt(ch).write_value(0);
        }
        ...
        // Unmask only after stale hardware and software events are gone.
        critical_section::with(|_| {
            r.haintmsk().modify(|w| {
                w.set_haintm(w.haintm() | (1 << ch));
            });
    ...
    async fn wait_for_periodic_slot(&mut self) {
        let now = Instant::now();
        if self.next_periodic_poll > now {
            Timer::at(self.next_periodic_poll).await;
        }
        self.next_periodic_poll = Instant::now() + Duration::from_millis(self.interval_ms.max(1) as u64);
    }
    ...
    fn classify_events(events: u16) -> ChannelEvent {
        ...
        } else if events & EV_TXERR != 0 {
            ChannelEvent::TransactionError
        } else if events & EV_FRMOR != 0 {
            ChannelEvent::FrameOverrun
    ...
    async fn do_out_transfer(&mut self, ep_type: EndpointType, data: &[u8], dpid: u8) -> Result<(), PipeError> {
        ...
        loop {
            ...
            match Self::classify_events(events) {
                ...
                ChannelEvent::Error(error) => return Err(error),
            }
        }
    }
        ...
        loop {
            ...
            match Self::classify_events(events) {
                ...
                ChannelEvent::TransactionError | ChannelEvent::FrameOverrun if is_periodic => {
                    self.periodic_error_count = self.periodic_error_count.saturating_add(1);
                    self.halt_channel();
                    if self.periodic_error_count >= 3 {
                        self.periodic_error_count = 0;
                        return Err(PipeError::BadResponse);
                    }
                    continue;
